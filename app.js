...
app.post("/newuser", async (req, res) => {
  ...
  const { user_id, email, given_name, family_name, latitude, longitude } =
    req.body;
  const output = await createUserProfile(
    user_id,
    email,
    given_name,
    family_name,
...
app.use("/didtheyanswertoday", async (req, res) => {
  console.log(
    "/didtheyanswertoday triggered - attempting to fetch users most recent response"
  );
  const user = req.query.user_id;
...
app.post("/storeexpopushtoken", async (req, res) => {
...
app.listen(process.env.PORT || 3030, () => {
  console.log(
    "Node JS Server started on 3030.  ----  If running locally, make sure Docker is running on 3306, and localtunnel is exposing 3306."
  );
});
